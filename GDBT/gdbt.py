...
import numpy as np
import logging
from GDBT.decision_tree import Decision_tree
...
class BaseGradientBoosting(AbstractBaseGradientBoosting):
    ...
    def __init__(self,learning_rate, n_trees, max_depth, feature_number,lr_decay = None):#lr_decay学习率衰减   每过lr_decay个循环  学习率减少0.1
        super().__init__()
    ...
    def fit(self,all_imgs, target,test_imgs= None,test_label=None):
        ...
        for iter in range(1, self.n_trees+1):
            ...
            self.trees[iter] = Decision_tree(all_imgs=all_imgs, target=res_target, max_depth=self.max_depth, logger=logger,feature_number=self.feature_number).nodelist
            self.f_value[iter]=copy.deepcopy(self.f_value[iter-1])
            for idx , img in enumerate(all_imgs):
                self.f_value[iter][idx] =self.f_value[iter][idx]+ self.learning_rate*(self.trees[iter][0].predict(img))

            train_loss = 0
            for idx, img in enumerate(all_imgs):
                pre_label = self.predict(img)
                train_loss += np.mean(np.square(target[idx] - pre_label))
            train_loss = train_loss / len(all_imgs)
            ...
            plt.title("Loss plot")
            plt.legend()
            plt.savefig('../log/{}_{}_{}_{}_GDBT_loss.jpg'.format(self.learning_rate_cp,self.n_trees,self.max_depth,self.feature_number))
            plt.close('all')
            model_path = '../model/GDBT_{}_{}_{}_{}.txt'.format(self.learning_rate_cp,self.n_trees,self.max_depth,self.feature_number)
    ...
    def predict(self,img):
        ...
        for iter in range(1, len(self.trees)+1):
            ...
            result += lr*self.trees[iter][0].predict(img)
...
if __name__ =="__main__":
    ...
    test_imgs, test_landmarks = getdata(test_path, 500)
    ...
    testB.fit(train_imgs,train_label,test_imgs,test_label)
    etime = time.time()
