...
class Node:
    ...
    def predict(self,feature):
        ...
        if  not self.left_child:
            ...
            f_v = feature[self.split_feature_index]
            if f_v<self.split_value:
                return self.left_child.predict(feature)
            else:
                return  self.right_child.predict(feature)


class Decision_tree:
    def __init__(self,all_data,target,max_depth,logger):
        ...
        self.rootnode = self.build_tree(range(len(all_data)), 0, 1)
    ...
    def find_best_fearture(self,data_index):
        ...
        for idx in range(self.data.shape[1]):
            ...
            for fea_val in range(int(min(tmp_feature)),int(max(tmp_feature)),1):
    ...
    def find_best_fearture2(self, data_index):
        ...
        for idx in range(self.data.shape[1]):
            ...
            tmp_feature = tmp_data[:, idx]
            begin = int(min(tmp_feature))
            end = int(max(tmp_feature))+1
...
def getdata(path,img_number):
    ...
    for code in codelist[:img_number]:
        ...
        with open(os.path.join(path, code + '.hogtxt'), 'r')as r:
            line = r.readline()
            hog_feature = [float(f) for f in line.split(',')]
        ...
        #     print(img.shape)
        #     showpic(img)
        all_features.append(hog_feature)
        all_imgs.append(img)
        all_landmarks.append(points)
    return all_imgs, all_landmarks,all_features
...
if __name__ =="__main__":
    ...
    # all_imgs1, all_landmarks1 ,all_feartures= getdata(train_path,10000)
    ...
    # dirlist = os.listdir(train_path)
    # codelist = [dirname.split(".")[0] for dirname in dirlist]
    # codelist = list(set(codelist))
    # for code in codelist:
    #     # 读取图片和镜像图片
    #     img = cv2.imread(os.path.join(train_path, code + '.jpg'))  # 读取图片
    #     img = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)  # 灰度处理
    #     # print(img.shape)  # 行数 列数 像素数
    #     # 读取坐标点
    #
    ...
    #     print(feature.shape)
    #     with open(os.path.join(train_path, code + '.hogtxt'), 'w')as w:
    #         strlist= [str(f) for f in list(feature)]
    #         w.write(','.join(strlist))
