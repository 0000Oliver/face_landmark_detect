...
class GSDM3_model():#不同于论文的测试集划分的方法，便利所有的DM暴力计算找到最优路径
    def __init__(self,leaning_rate,feature_block):
        ...
        self.loss = 100
        self.pca_base= []
        self.name= 'GSDM3_{}_{}'.format(self.learning_rate,self.feature_block)
    ...
    def Descent(self, imgs, state, label):
        ...
        '''
        delta_X = label - state
        # phi = [get_hog_feature(imgs[idx], label_to_landmark(state[idx]),self.feature_block) for idx in
        #        range(len(imgs))]  # (4-1)*(4-1)*9*4*12 =3888
        # phi = np.array(phi)
    ...
    def  fit(self,imgs,labels,test_imgs = None,test_labels=None):
        ...
        while self.loss>0.1:
            ...
            if train_loss > test_loss_list[-1]-0.04:
    ...
    def predict(self,img, pre_label,show = False):
        ...
        # phi_star = get_hog_feature(img, label_to_landmark(pre_label), self.feature_block)
        for i in range(len(self.derection)):

            # deta_X = pre_label-state
            # deta_phi = phi_star - get_hog_feature(img, label_to_landmark(state),self.feature_block)
            ...
            for subindex in range(8):
                tmpstate = state+self.learning_rate * self.compute_strid(img, state, self.derection[i][subindex])
                tmploss = self.compute_loss(tmpstate,pre_label)
                if tmploss<newloss:
                    newstate = tmpstate
                    newloss = tmploss
            state = newstate
...
if __name__ =="__main__":
    ...
    test_GSDM = GSDM3_model(LEARNING_RATE,4)
    ...
    # test_GSDM = GSDM3_model(LEARNING_RATE,5)
    # test_GSDM.fit(train_imgs[:9000],train_labels[:9000],train_imgs[-3000:],train_labels[-3000:])
    # test_GSDM = GSDM3_model(LEARNING_RATE,6)
    # test_GSDM.fit(train_imgs[:9000],train_labels[:9000],train_imgs[-3000:],train_labels[-3000:])
