...
class PCA(object):
    ...
    def reduce_dimension(self):
        ...
        if self.n_components:  # 指定降维维度
            ...
            print(np.dot(self.x,np.transpose(p)).shape)
...
class GSDM_model():
    def __init__(self,leaning_rate,feature_block,blocksize =8):
        ...
        self.loss = 100
        self.pca_base= []
        self.name= 'GSDMtest_{}_{}'.format(self.learning_rate,self.feature_block)
    ...
    def devide_subset(self,deta_X,deta_phi):
        ...
        self.logger.debug("降维，分类完成，耗时{}".format((endtime-starttime).seconds))
...
if __name__ =="__main__":
    ...
    train_imgs, train_landmarks = getdata2(1000)
    train_labels = landmark_to_label(train_landmarks)
    train_labels = np.array(train_labels)
    ...
    test_GSDM = GSDM_model(LEARNING_RATE,3)
    test_GSDM.fit(train_imgs[:100],train_labels[:100],train_imgs[-10:],train_labels[-10:])
    # test_GSDM = GSDM_model(LEARNING_RATE,5)
